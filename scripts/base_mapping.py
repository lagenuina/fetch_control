...
class baseMapping():
    def __init__(self, name):
        ...
        # Init keys
        # Fast motion
        ksm.keyStateMachine(parent_name=self.name, key_name='w', on_press=self.set_vel, on_release=self.reset_vel, type="lin", vel=self.max_lin_vel)
        ksm.keyStateMachine(parent_name=self.name, key_name='s', on_press=self.set_vel, on_release=self.reset_vel, type="lin", vel=-1*self.max_lin_vel*0.5)
        ksm.keyStateMachine(parent_name=self.name, key_name='a', on_press=self.set_vel, on_release=self.reset_vel, type="rot", vel=self.max_rot_vel)
    ...
    def pub_vel(self):
        ...
        self.current_lin_vel = self.update_vel(self.target_lin_vel, self.current_lin_vel, self.lin_accel)
        self.current_rot_vel = self.update_vel(self.target_rot_vel, self.current_rot_vel, self.rot_accel)
